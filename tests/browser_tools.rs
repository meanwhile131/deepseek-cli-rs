...
async fn test_browser_tools() -> Result<()> {
    let test_future = async {
        ...
        assert!(
            res.0.contains("Clicked element"),
        ...
        .await?;
        assert!(
            status.0.contains("Clicked"),
            "browser_evaluate after click did not return updated text: {status:?}"
        );

        // Open a new tab
        let res = execute_tool("browser_new_tab", "about:blank").await?;
        assert!(
            res.0.contains("Opened new tab 2"),
        ...
        assert!(
            res.0.contains("Switched to tab 1"),
